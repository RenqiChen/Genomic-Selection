...
from dataset_wheat_45 import fetch_dataset
...
def test_one_epoch(i_epoch, loader, model, criterion, args):
    ...
    for step, data in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            predicts += list(output.detach().cpu().squeeze(-1).numpy())
            targets += list(label.detach().cpu().numpy())
            total_loss += loss.detach().cpu().item()

    total_loss /= len(loader)
    pearson_ce, _ = pearsonr(predicts, targets)
    print(f"test epoch[{i_epoch}/{args.epoch}] test loss:{total_loss} || pearson:{pearson_ce}")
    return {"loss": total_loss, "pearson": pearson_ce}
...
def normal_train_test(train_loader, test_loader, model, optimizer, criterion, args):
    ...
    for i in tqdm(range(1, epoch+1)):
        ...
        writer.add_scalar('Test/pearson',test_result["pearson"], i)
        if best_metric < test_result["pearson"]:
            best_metric = test_result["pearson"]
